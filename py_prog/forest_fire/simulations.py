...
def multiple_simulations(nb_simus, size, b, p, max_time, diag_neigh, seed=None):
    ...
    Q2 = nb_simus // 2
    Q3 = 3*nb_simus // 4
    return  round( sum(nb_burnt_trees)/(nb_simus*size*size) ,3)   ,  sum(nb_timesteps)/nb_simus
...
def variable_proba_propagation(nb_simus, size, b, list_of_p, max_time, diag_neigh, seed=None):
    """
    ...
    """
    for p in list_of_p:
        s=    multiple_simulations(nb_simus, size, b, p, max_time, diag_neigh, seed)
        s = s+ (p,)
        print(s, sep=",")
        #print(p, multiple_simulations(nb_simus, size, b, p, max_time, diag_neigh, seed),sep=",")
