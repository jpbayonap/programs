...
while not done:
	...
	if any([b == remainder for b in mel[:count-1] ]) : #when computing the division of one and an integer  the values admitted for quotient are restricted
	#to 1,...,d . hence if in the computation  there is two values of the remainger variable  which coincide then  the entries of myli between them will be repeated and the
	#code computation must be stopped

		done= True
myli[count]=(x*10)//d  #fill the last digit of the repeating sequence

my= [str(a)for a in myli]   #convert the list of integer into a string for printing purposes
print("the result is")
print('0.'+''.join(my[:count]) + '('+''.join(my[my.index(my[count]):count])+')') #.index command is used to determine the place in the array where there is the other repeated value
